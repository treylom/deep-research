...
구 7-Phase 파이프라인 스킬 2종·에이전트 1종·A–E 등급 문서는 3.0.2 에서 제거됐습니다(스킬 목록에 구 이름이 함께 뜨는 혼동 때문). 구 명령 `/deep-research` 는 3.0.3 에서 제거됐습니다 — `/searchflow` 를 쓰세요. 구 run 원장은 그대로 읽을 수 있고, 필요하면 git 이력(3.0.2 로 올린 커밋 이전)에서 복원합니다.
