...
- **소스 등급 4단**: **원Source · A급 · B급 · C급** — 매체가 어디인지가 아니라 **"이 URL 이 그 주장에 대해 무엇인가"** 로 정합니다. 같은 신문이라도 원문을 그대로 실으면 원Source, 해설이면 C. (여기에 **`사용불가`** 가 하나 더 붙는데 그건 등급이 아니라 **처분** — 등급은 그대로 두고 근거로 안 쓰는 것입니다. 못 읽은 것은 또 다른 축인 `UNREACHABLE` 로 갑니다)
